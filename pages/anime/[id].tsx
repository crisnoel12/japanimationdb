  ...
  return (
    ...
    <style jsx global>{`
      ...
      <div className="container mx-auto mb-4 px-10">
        <div className={`grid grid-cols-12 ${styles.overlayContainer}`} style={{ 
          backgroundImage: data.attributes.coverImage && `url(${data.attributes.coverImage.original})`,
        ...
        <div className={`grid grid-cols-1 md:grid-cols-12`}>
          ...
          <div className='bg-slate-800 col-span-1 md:col-span-9 text-center p-8'>
            ...
            <p className='whitespace-pre-wrap tracking-wide'>{data.attributes.synopsis.replace(/(?:\r\n|\r|\n)/g, "\n\n")}</p>
          </div>
        </div>
      </div>
    </>
...
export async function getServerSideProps(context: NextPageContext) {
  const response = await fetch(`https://kitsu.io/api/edge/anime/${context.query.id}`)
